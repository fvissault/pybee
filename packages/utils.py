...
class Utils:
    ...
    def err(self, code, context):
        return core_errors.__dict__[code].print_error(context, self.interpreter)
    ...
    def logon_instr(self):
        if not self.interpreter.activelog:
            ...
            try:
                ...
                self.loginfo('Exec utils log-on instruction')
            except OSError:
                return file_errors.error_open_file_failed.print_error('log-on', self.interpreter)
        return 'nobreak'

    def logoff_instr(self):
        self.loginfo('Exec utils log-off instruction')
        if self.interpreter.activelog:
            self.interpreter.activelog = False
            if self.interpreter.logdescriptor is None:
                return file_errors.error_not_a_file_descriptor.print_error('log-off', self.interpreter)
        ...
        if self.interpreter.activelog:
            ...
            if os.path.exists(file_path):
                ...
                try:
                    ...
                    return file_errors.error_open_file_failed.print_error('log-info', self.interpreter)
        ...
        if self.interpreter.activelog:
            ...
            if os.path.exists(file_path):
                ...
                try:
                    ...
                    return file_errors.error_open_file_failed.print_error('log-warn', self.interpreter)
        ...
        if self.interpreter.activelog:
            ...
            if os.path.exists(file_path):
                ...
                try:
                    ...
                    return file_errors.error_open_file_failed.print_error('log-err', self.interpreter)
