...
class web(base_module):
    def __init__(self, interpreter):
        ...
        self.dictionary = {
            ...
            'addattr' : '''local valueattr local nameattr local contattrs nameattr @ valueattr @ contattrs @ "attrs" cell@ cell+ drop''',
            'fieldarea' : '''{ } local attrs local fatype local favalue local faid "type" fatype @ attrs cell+ drop favalue @ "" <> if "value" favalue @ attrs cell+ drop then faid @ "" <> if "id" faid @ attrs cell+ drop "name" faid @ attrs cell+ drop then { tag : "input" , content : [ ] , attrs : attrs @ , container : "n" }''',
            ...
            'a' : '''local href local cont { } local attrs "href" href @ attrs cell+ drop { tag : "a" , content : [ cont @ ] , attrs : attrs @ , container : "y" }''',
            ...
            'th' : '''local tableid local colnum local properties properties @ 0 cell@ local cont properties @ 1 cell@ local scope properties @ 2 cell@ local filter properties @ 3 cell@ local sort properties @ cells 6 = if properties @ 4 cell@ local rowspan properties @ 5 cell@ local colspan then { } local attrs scope @ "col" = scope @ "row" = or if "scope" scope @ attrs cell+ drop then properties @ cells 6 = if rowspan @ 1 > if "rowspan" rowspan @ attrs cell+ drop then colspan @ 1 > if "colspan" colspan @ attrs cell+ drop then then { tag : "th" , content : [ cont @ ] , attrs : attrs @ , container : "y" } local tag tag "class" "tablehead" addattr sort @ "sort" = if "userarea/img/sort-solid-full.svg" img local sortimage sortimage "width" 16 addattr sortimage "onclick" "sort(\"<#0#>\", this, <#1#>)" [ tableid @ colnum @ ] format addattr sortimage "style" "vertical-align:middle;" addattr tag sortimage @ addcontent then filter @ "filter" = if tag br addcontent "mytablef<#0#>" [ colnum @ ] format "" "text" fieldarea local fa fa "class" "filterinput" addattr fa "onkeyup" "filter(\"<#0#>\", this, <#1#>)" [ tableid @ colnum @ ] format addattr fa "placeholder" "Filter..." addattr tag fa @ addcontent then tag @''',
            'td' : '''local properties properties @ 0 cell@ local cont properties @ cells 3 = if properties @ 1 cell@ local rowspan properties @ 2 cell@ local colspan then { } local attrs properties @ cells 3 = if rowspan @ 1 > if "rowspan" rowspan @ attrs cell+ drop then colspan @ 1 > if "colspan" colspan @ attrs cell+ drop then then { tag : "td" , content : [ cont @ ] , attrs : attrs @ , container : "y" } local cell cell "class" "tablecell" addattr cell @''',
            'row' : '''local properties local container container @ "attrs" cell@ "id" cell@ local tableid 0 local i 0 local item tablerow local tr properties @ cells i do properties @ i @ cell@ item ! item @ cells 4 = item @ cells 6 = or if tr item @ i @ tableid @ th addcontent else tr item @ td addcontent then loop container @ tr @ addcontent''',
            ...
            'completeselect' : '''select local tempselect 0 local i 0 local item 0 local opt dup cells i do dup i @ cell@ item ! item @ 0 cell@ item @ 1 cell@ item @ 2 cell@ option opt ! "content" opt @ tempselect cell+ drop loop drop tempselect @''',
            ...
            'splitter' : '''local direction local name local container direction @ "vertical" = direction @ "horizontal" = or if direction @ "vertical" = if "<#0#>left" [ name @ ] format local firstid "<#0#>right" [ name @ ] format local secondid else "<#0#>top" [ name @ ] format local firstid "<#0#>bottom" [ name @ ] format local secondid then "<#0#>divider" [ name @ ] format local dividerid firstid @ div "var <#0#>" [ firstid @ ] format evaluate direction @ "vertical" = if firstid @ "class" "splitleft" addattr else firstid @ "class" "splittop" addattr then secondid @ div "var <#0#>" [ secondid @ ] format evaluate direction @ "vertical" = if secondid @ "class" "splitright" addattr else secondid @ "class" "splitbottom" addattr then dividerid @ div "var <#0#>" [ dividerid @ ] format evaluate direction @ "vertical" = if dividerid @ "class" "splitverticaldivider" addattr else dividerid @ "class" "splithorizontaldivider" addattr then container @ firstid @ addcontent container @ dividerid @ addcontent container @ secondid @ addcontent direction @ "vertical" = if container @ "defer" "const <#0#>=new Splitter(\"<#1#>\",\"<#2#>\",\"vertical\");" [ name @ dividerid @ firstid @ ] format script addcontent else container @ "defer" "const <#0#>=new Splitter(\"<#1#>\",\"<#2#>\",\"horizontal\");" [ name @ dividerid @ firstid @ ] format script addcontent then else container @ "Bad direction for your splitter : only vertical or hoizontal allowed" addcontent then''',
            ...
            'tab' : '''local dir local name local defaultopen local buttontab local container dir @ "top" = dir @ "left" = or if name @ div local buttondiv buttondiv "class" "<#0#>tab" [ dir @ ] format addattr 0 local i 0 local value 0 local color 0 local item buttontab @ cells i do "<#0#>tab<#1#>" [ name @ i @ ] format local buttonid buttontab @ i @ cell@ item ! item @ 0 cell@ value ! item @ 1 cell@ color ! value @ "button" button local bouton i @ defaultopen @ = if "<#0#>tablinks defaultOpen" [ dir @ ] format local boutonclass else "<#0#>tablinks" [ dir @ ] format local boutonclass then bouton "class" boutonclass @ addattr bouton "onclick" "openTab(event,\"<#0#>\",this,\"<#1#>\",\"<#2#>\")" [ buttonid @ color @ dir @ ] format addattr buttondiv @ bouton @ addcontent loop container @ buttondiv @ addcontent 0 local tabid 0 i ! buttontab @ cells i do "<#0#>tab<#1#>" [ name @ i @ ] format tabid ! tabid @ div "var <#0#>" [ tabid @ ] format evaluate tabid "class" "<#0#>tabcontent" [ dir @ ] format addattr container @ tabid @ addcontent loop container "onload" "showDefault()" addattr else container @ "Bad direction for your tab : only top or left allowed" addcontent then''',
            'accordion' : '''local name local sections local container 0 local i 0 local panelname 0 local item 0 local bouton sections @ cells i do sections @ i @ cell@ item ! item @ "button" button bouton ! bouton "class" "accordion" addattr container @ bouton @ addcontent "<#0#>panel<#1#>" [ name @ i @ ] format panelname ! panelname @ div "var <#0#>" [ panelname @ ] format evaluate panelname @ "class" "panel" addattr container @ panelname @ addcontent loop''',
            'sidenav' : '''local name local sntype local links local container sntype @ "push" = sntype @ "overlay" = or if "<#0#>sidenav" [ name @ ] format local snname "sidenav" div "var <#0#>" [ snname @ ] format evaluate snname @ "class" "sidenav" addattr container @ snname @ addcontent "&times;" "#" a local closebtn closebtn @ "id" "closebtn" addattr closebtn @ "class" "closebtn" addattr sntype @ "push" = if closebtn @ "onclick" "closeNavPush()" addattr else closebtn @ "onclick" "closeNavOverlay()" addattr then snname @ closebtn @ addcontent "&#9776;" "#" a local openbtn openbtn @ "id" "openbtn" addattr openbtn @ "class" "openbtn" addattr sntype @ "push" = if openbtn @ "onclick" "openNavPush()" addattr else openbtn @ "onclick" "openNavOverlay()" addattr then snname @ openbtn @ addcontent 0 local i 0 local item 0 local content 0 local href 0 local link links @ cells i do links @ i @ cell@ item ! item @ 1 cell@ content ! item @ 0 cell@ href ! content @ href @ a link ! link "onclick" "openLink(event)" addattr link "class" "navlink" addattr snname @ link @ addcontent loop "<#0#>main" [ name @ ] format local snmain "main" div "var <#0#>" [ snmain @ ] format evaluate container @ snmain @ addcontent else container @ "Bad type for your sidenav : only push or overlay allowed" addcontent then''',
            'topnav' : '''local name local defaultopen local tntype local links local container "topnav" local tnclass tntype @ "fixed" = if "topnavfixed" tnclass ! then "<#0#>topnav" [ name @ ] format local tnname tnname @ div "var <#0#>" [ tnname @ ] format evaluate tnname @ "class" tnclass @ addattr 0 local item 0 local content 0 local href 0 local link 0 local i links @ cells i do links @ i @ cell@ item ! item @ 0 cell@ content ! item @ 1 cell@ href ! content @ href @ a link ! defaultopen @ i @ = if link "class" "navlink defaultOpen" addattr else link "class" "navlink" addattr then link "onclick" "openLink(event)" addattr tnname @ link @ addcontent loop container @ tnname @ addcontent container @ "onload" "showDefault()" addattr''',
            'filteredlist' : '''local name local itemlist local container name @ unorderlist local ulist ulist "class" "unorderedlist" addattr 0 local i itemlist @ cells i do ulist itemlist @ i @ cell@ listitem addcontent loop "<#0#>_filter" [ name @ ] format "" "text" fieldarea local fa fa "class" "filterinput" addattr fa "placeholder" "Filter..." addattr fa "onkeyup" "listfilter(\"<#0#>\", this);" [ name @ ] format addattr container @ fa @ addcontent container @ ulist @ addcontent''',
            'modal' : '''local name local title local btncontent local container btncontent @ "nobutton" <> if btncontent @ "button" button local btn btn "onclick" "openmodal(\"<#0#>\", this)" [ name @ ] format addattr container @ btn @ addcontent then name @ div local modtemp modtemp "class" "modal" addattr "<#0#>content" [ name @ ] format local modcontentname modcontentname @ div "var <#0#>" [ modcontentname @ ] format evaluate modcontentname @ "class" "modalcontent" addattr modtemp modcontentname @ addcontent "<#0#>title" [ name @ ] format div local modtitle modcontentname @ modtitle @ addcontent title @ span local spantitle spantitle "class" "modaltitle" addattr modtitle spantitle @ addcontent "&nbsp;&times;&nbsp;" span local modclose modclose "class" "modalclose" addattr modclose "id" "<#0#>close" [ name @ ] format addattr modtitle modclose @ addcontent container @ modtemp @ addcontent''',
            'advancebutton' : '''local name local content local type local look local container 0 local btnclass look @ "fullsuccess" = look @ "fullinfo" = or look @ "fullwarning" = or look @ "fulldanger" = or look @ "fulldefault" = or local fullbtn look @ "outlinesuccess" = look @ "outlineinfo" = or look @ "outlinewarning" = or look @ "outlinedanger" = or look @ "outlinedefault" = or local outlinebtn fullbtn @ outlinebtn @ or if type @ "button" = type @ "reset" = or type @ "submit" = or if fullbtn @ if "<#0#> <#1#>" [ "fullbtn" look @ ] format btnclass ! then outlinebtn @ if "<#0#> <#1#>" [ "outlinebtn" look @ ] format btnclass ! then content @ type @ button "var <#0#>" [ name @ ] format evaluate name @ "class" btnclass @ addattr name @ "id" name @ addattr container @ name @ @ addcontent else container @ "Bad type for your button : only button, reset, submit are allowed" addcontent then else container @ "Bad look for your button : only fullsuccess, fullinfo, fullwarning, fulldanger, fulldefault, outlinesuccess, outlineinfo, outlinewarning, outlinedanger, outlinedefault are allowed" addcontent then''',
            'loginform' : '''local passlostaction local formaction local container load translations/loginform formaction @ "post" form local formulaire formulaire "id" "loginform" addattr "container" div local contdiv contdiv "class" "container" addattr contdiv 2 "identification" _loginform maintitle addcontent contdiv "email" _loginform bold "email" label addcontent "email" "" "email" fieldarea local emailtf emailtf "placeholder" "enteremail" _loginform addattr emailtf "required" "required" addattr contdiv emailtf @ addcontent contdiv "password" _loginform bold "password" label addcontent "password" "" "password" fieldarea local passtf passtf "placeholder" "enterpassword" _loginform addattr passtf "required" "required" addattr contdiv passtf @ addcontent contdiv 3 "login" _loginform maintitle "submit" button addcontent "rememberme" "" "checkbox" fieldarea local remembercb remembercb "checked" "checked" addattr contdiv remembercb @ addcontent contdiv "rememberme" _loginform addcontent "forgot" _loginform span local spanzone spanzone "class" "psw" addattr spanzone "pass?" _loginform passlostaction @ a addcontent contdiv spanzone @ addcontent formulaire contdiv @ addcontent container @ formulaire @ addcontent''',
            'signupform' : '''local t&p local action local container load translations/signupform action @ "post" form local formulaire formulaire "id" "signupform" addattr formulaire "class" "signupform" addattr formulaire "data-msg-passwordmismatch" "passwordmismatch" _signup addattr "container" div local contdiv contdiv "class" "container" addattr contdiv 1 "Signup" maintitle addcontent contdiv "recommend" _signup paragraph addcontent contdiv hr addcontent contdiv "firstname" _signup bold "firstname" label addcontent "firstname" "" "text" fieldarea local firstname firstname "placeholder" "enterfn" _signup addattr firstname "required" "required" addattr contdiv firstname @ addcontent contdiv "lastname" _signup bold "lastname" label addcontent "lastname" "" "text" fieldarea local lastname lastname "placeholder" "enterln" _signup addattr lastname "required" "required" addattr contdiv lastname @ addcontent contdiv hr addcontent contdiv "email" _signup bold "email" label addcontent "email" "" "email" fieldarea local emailtf emailtf "placeholder" "enteremail" _signup addattr emailtf "required" "required" addattr contdiv emailtf @ addcontent contdiv "password" _signup bold "password" label addcontent "password" "" "password" fieldarea local passtf passtf "placeholder" "enterpassword" _signup addattr passtf "required" "required" addattr contdiv passtf @ addcontent contdiv "repeat" _signup bold "repeat" label addcontent "repeat" "" "password" fieldarea local repeattf repeattf "placeholder" "repeat" _signup addattr repeattf "required" "required" addattr contdiv repeattf @ addcontent "rememberme" "" "checkbox" fieldarea local remembercb remembercb "checked" "checked" addattr contdiv remembercb @ addcontent contdiv "rememberme" _signup addcontent "agree" _signup paragraph local agree agree "tandp" _signup t&p @ a addcontent contdiv agree @ addcontent contdiv 3 "signup" _signup maintitle "submit" button addcontent formulaire contdiv @ addcontent container @ formulaire @ addcontent'''
        }
