...
class mysqldb(base_module):
    def __init__(self, interpreter):
        ...
        self.dictionary = {
            ...
            'addforeignkey' : '''local cascade local origtabname local desttabname "|alter> table <#0#> add constraint fk_<#0#>_<#1#> foreign key (id_<#1#>) references <#1#>(id)" [ desttabname @ origtabname @ ] format local req "delete" cascade @ scan if " on delete cascade" req s+! then "update" cascade @ scan if " on update cascade" req s+! then " >|" req s+! req @ evaluate''',
